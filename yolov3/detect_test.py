...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.5, debug=False):
    """
    ...
    for j in range(num):
        ...
        for i in range(meta.classes):
            ...
            if dets[j].prob[i] > 0:
                ...
                res.append((nameTag, dets[j].prob[i], (b.x - b.w / 2, b.y - b.h / 2, b.x + b.w / 2, b.y + b.h / 2)))
...
def vis_detections(im, boxes, scores):
    """Visual debugging of detections."""
    for score, box in zip(scores, boxes):
        bbox = tuple(int(np.round(x)) for x in box)

        cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 255, 0), 6)
        cv2.putText(im, 'bad: %.3f' % (score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
...
if __name__ == "__main__":
    ...
    def result2file(boxes, file_name):
        print("!!!!!!", file_name)
        with open(f'txt_result/{file_name}.txt', 'a', encoding='utf-8') as file:
            for box in boxes:
                file.write(f'bad {box[0]} {box[1]} {box[2]} {box[3]}\n')


    # global metaMain, netMain, altNames
    print(performDetect(imagePath='/disk2/zhaoliang/Projects/darknet/darknetAB/cloth/J01_2018.06.28 14_54_13.jpg',
                        configPath='cloth/train.cfg',
                        metaPath='cloth/voc.data',
                        weightPath='cloth/backup/train_2376.weights',
                        thresh=0.25,
                        showImage=False
                        ))

    config_path = "cloth/train.cfg"
    weight_path = "cloth/backup/train_2376.weights"
    meta_path = "cloth/voc.data"
    # detector = YoloModel(config_path, weight_path, meta_path, 0.05)
    # root = '/disk5/zhaoliang/ClothDataSet/VOC2007/JPEGImages'
    root = '/disk_tmp/VOC_Cloth/JPEGImages'
    # root = "/disk5/zhaoliang/5.20"
    # total_num = len(os.listdir(root))
    # for index, file_name in enumerate(os.listdir(root)):
    with open('/disk_tmp/VOC_Cloth/ImageSets/Main/test.txt', 'r') as f:
        lines = f.readlines()
        total_num = len(lines)
        for index, file_name in enumerate(lines):
            try:
                ...
                for idx, (crop_img, point) in enumerate(zip(crop_imgs, points)):
                    ...
                    score = []
                    p = []
                    tmp_path = 'tmp_img/tmp.jpg'
                    imsave(tmp_path, crop_img)
                    ...
                    #                     showImage=False
                    #                     )
                    res = detect(netMain, metaMain, tmp_path.encode("ascii"), 0.02, nms=.5)
                    if len(res) > 0:
                        for det in res:
                            score.append(det[1])
                            # print(det[2])
                ...
                from merge_result_momentum import MergeTool

                mt = MergeTool(class_num=2, edge_alpha=1)
                boxxx = np.column_stack([final_boxes, np.zeros((len(final_boxes), 1)), final_scores])
                import time

                s = time.time()
                boxxx = mt.merge(boxxx)
                t = time.time()
                # print(t - s)
                boxxx = mt.merge(boxxx)
                # print(time.time() - t)
                result2file(boxxx, file_name)
                print(f'{index + 1}/{total_num}  {filename}  result num : {len(boxxx)}')
                # im = cv2.imread(img_path)
                # im = vis_detections(im, final_boxes, final_scores)
                # # if len(final_scores) > 0:
                # cv2.imwrite(f'detect_result/res{filename}', im)
            except Exception as e:
                print(e)

    # filename = 'J01_2018.06.17_15_07_25.jpg'
    # img_path = os.path.join(root, filename)
    # img = imread(img_path)
    # crop_imgs, points = crop_ori_img_scale(img)
    # final_scores = []
    ...
    #     final_boxes, final_scores, final_points
    # )
    # print(f'{1 + 1}/{total_num}  {filename}  result num : {len(final_scores)}')
    # im = cv2.imread(img_path)
    # from merge_result_momentum import MergeTool
    #
    # mt = MergeTool(class_num=2, edge_alpha=1)
    ...
    # boxxx = mt.merge(boxxx)
    # print(time.time()-t)
    # im = vis_detections(im, boxxx[:, :4], boxxx[:, 5])
    # print(boxxx)
    # # if len(final_scores) > 0:
    # cv2.imwrite(f'detect_result/merge_res{filename}', im)
